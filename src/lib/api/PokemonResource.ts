...
import type { GenerationalSprites, GenerationalSpritesResource } from '$lib/api/SpritesResource'
...
export async function getPokemon(
  ...
  return getResource<PokemonResource>(POKEMON_ENDPOINT, identifier, fetchCallback).then(
    async (pokemonResource) => ({
      ...
      sprites: {
        ...
        versions: Object.fromEntries(
          Object.entries(pokemonResource.sprites.versions).map(([generation, next]) => [
            generation,
            Object.fromEntries(
              Object.entries(next).map(([versionGroup, spritesResource]) => [
                versionGroup,
      ...
      forms: await Promise.all(
        pokemonResource.forms.map((formLinkResource) =>
          getPokemonForm(formLinkResource.name, fetchCallback)
        )
      )
    })
  )
