...
export async function getPokemonSpecies(identifier: Identifier, fetchCallback: FetchFunction = fetch): Promise<PokemonSpecies> {
	return getResourceWithCache<PokemonSpeciesResource>(POKEMON_SPECIES_ENDPOINT, POKEMON_SPECIES_CACHE, identifier, fetchCallback)
		.then(async pokemonSpeciesResource => ({
			...
			varieties: await Promise.all(
				pokemonSpeciesResource.varieties.map(pokemonLinkResource => getPokemon(pokemonLinkResource.pokemon.name, fetchCallback))
			)
		}))
}
