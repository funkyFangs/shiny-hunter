...
export function isVarietySupported(variety: string, generation: number, versionGroup: string): boolean {
	...
	return !(
		VARIETY_BLACKLIST.has(variety)
			...
			|| (variety.match(/^kyurem-(black|white)$/) && generation === 5 && versionGroup !== 'black-2-white-2')
			|| (variety.endsWith('-resolute') && generation === 5 && versionGroup !== 'black-2-white-2')
			// ORAS & onwards
			|| (variety.endsWith('-primal') && (generation < 6 || (generation === 6 && versionGroup !== 'omegaruby-alphasapphire')))
			|| (variety.endsWith('-unbound') && generation === 6 && versionGroup !== 'omegaruby-alphasapphire')
			// SM & onwards
			...
			|| (variety.match(/^necrozma-(dusk|dawn|ultra)$/) && generation === 7 && versionGroup !== 'ultra-sun-ultra-moon')
			// BDSP
