...
export async function getVersionGroup(identifier: Identifier, fetchCallback: FetchFunction = fetch): Promise<VersionGroup> {
	return getResource<VersionGroupResource>(VERSION_GROUP_ENDPOINT, identifier, fetchCallback)
		.then(versionGroupResource => ({
