<script lang="ts">
  ...
  const loadedGames = loadedGamesJson.map(
    (loadedGame) =>
      new Game(
  ...
  const gameUnsubscribe = selectedGame.subscribe((game) => {
    if (game) {
      ...
      fetch(game.pokemonFile)
        ...
        .then((array) =>
          array.map(
            ({ displayName, image, shinyImage, icon, variants }) =>
              new Pokemon(
                displayName,
                variants?.length
                  ? undefined
                  : `${image || displayName}`.replace(" ", "_"),
                variants?.length
                  ? undefined
                  : `${shinyImage || displayName}.shiny`.replace(" ", "_"),
                variants?.length
                  ? undefined
                  : `${icon || displayName}`.replace(" ", "_"),
                variants?.length
                  ? variants.map(
                      (variant) =>
                        new Variant(
                          variant.displayName,
                          `${variant.image || `${displayName}.${variant.displayName}`}`.replace(
                            " ",
                            "_",
                          ),
                          `${variant.shinyImage || `${displayName}.${variant.displayName}`}.shiny`.replace(
                            " ",
                            "_",
                          ),
      ...
      fetch(game.methodsFile)
        ...
        .then((array) =>
          array.map(
            ({ displayName, singularUnit, pluralUnit, odds }) =>
...
<div id="create-tracker">
  <form id="selections">
    ...
    <select
      ...
      disabled={$loadedPokemon.length <= 0}
    >
      <option selected style="display:none" />
    ...
    <select
      ...
      disabled={$loadedMethods.length <= 0}
    >
      <option selected style="display:none" />
    ...
    <input
      ...
      type="checkbox"
      id="shiny-charm"
      bind:checked={$shinyCharm}
      disabled={!$selectedGame?.supportsShinyCharm}
    />
  </form>

  <button
    id="create"
    disabled={!($selectedGame && $selectedPokemon && $selectedMethod)}
    on:click={createTracker}>Create Tracker</button
  >
</div>
