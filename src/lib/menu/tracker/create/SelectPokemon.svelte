<script lang="ts">
	import { formatPokemonSpeciesName, getPokemonSpecies, type PokemonSpecies } from '$lib/api/PokemonSpeciesResource';
	...
	$: filteredVarieties = selectedPokemonSpecies
		? selectedGeneration && selectedVersionGroup
			? selectedPokemonSpecies.varieties.filter(pokemon => isVarietySupported(pokemon.name, selectedGeneration.id, selectedVersionGroup.name))
...
<div id="pokemon-entry">
	...
	{#if selectedPokemonSpecies && selectedVersionGroup && isShinyLocked(selectedPokemonSpecies.name, selectedVersionGroup.name) && selectedVersion}
		<ToolTip id="shiny-lock-warning" title="This Pokemon is shiny-locked in Pokemon {formatVersionName(selectedVersion.name)}.">
			<span class="warning"/>
		</ToolTip>
	{/if}
</div>
	...
	{#if selectedGeneration && selectedVersionGroup && filteredVarieties && filteredVarieties.length > 1}
		...
		<select bind:value={selectedPokemon} id="variety">
			{#each filteredVarieties as variety}
				<option value={variety}>{formatPokemonName(selectedPokemonSpecies.name, variety.name)}</option>
			{/each}
		</select>
	{/if}
