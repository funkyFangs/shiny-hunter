...
export async function load({ fetch }) {
	...
	const currentTrackedPokemon = new Set(currentHuntTrackers.map(huntTracker => huntTracker!.pokemonSpecies))

	const generations: Generation[] = await Promise.all(
		GENERATION_NUMBERS.map(async generationNumber => getGenerationResource(generationNumber, fetch)
			.then(async generationResource => ({
				...
				pokemonSpecies: await Promise.all(
					generationResource.pokemon_species.map(
						async pokemonSpeciesLinkResource =>
	...
	if ((currentSelectedTrackerIndex < 0 || currentSelectedTrackerIndex >= currentHuntTrackers.length) && currentHuntTrackers.length > 0) {
